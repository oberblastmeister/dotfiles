    ...
    def run(self, flake: Path) -> None:
        ...
        try:
            if self.hardware_conf is None:
                ...
                shutil.copy(
                    self.hardware_conf, host_path / "hardware-configuration.nix"
                )
...
def get_parser() -> ArgumentParser:
    ...
    update_dconf_parser.add_argument(
        "--force", action="store_true", help="Override the file"
    )
    add_host_parser = subparsers.add_parser("add-host")
    add_host_parser.add_argument("hostname", type=str, help="Required hostname")
...
def is_flake_dir(path: Path) -> None:
    ...
    assert path.joinpath(
        "flake.nix"
    ).exists(), "flake.nix does not exist in the directory"
